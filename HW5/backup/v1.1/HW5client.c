...
int main(int argc, char* argv[])
{
  ...
  while ((fgets(line_data, sizeof(line_data), stdin) != NULL))
  {
    ...
    {
      c = line_data[i];
      //fails here the second time through...
      //presumably because the socket failed and the socket is connected
    ...
    for (i = 0; i < MAX_LINE; i++)
    {
      ...
      //printf("after client getc c = [%c]\n", c);
      if (c == EOF)
      {
        // if I comment all this out, it runs once...
        // but this is pretty clearly where the problem is
        // I think it's because I'm closing my sockets too early
        // but I literally went through every socket_close call and
        // commented it out and I still had this problem...
