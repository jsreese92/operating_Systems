...
/*
 * Sets up the service, ForkExecuteRedirect_helper actually executes code
 */
void ForkExecuteRedirect_service(void)
{
  int i, c, rc;
...
//      printf("character received is: [%c]\n", c);
      ...
      else
      {
        ...
        // TODO
        // here needs to be the code for processing arguments for execvp
        // Need to get all characters, place in input buffer, parse the 
        // arguments, and then pass those to FER_h

        if (c == '\n'){ // command entered
          //TODO make this look better, space shouldn't be needed
          input[input_index] = ' ';
          ...
          if (input_index <= INPUT_SIZE_MAX){
            ...
            //printf("added [%c] to input string\n", c);
            input_index++;
          }
    ...
    count = strlen(toClient) + 1; /* include '\0' in count */
    for (i = 0; i < count; i++)
    {
      c = toClient[i];
...
void ForkExecuteRedirect_helper(int argc, char *argv[]){
  ...
  { /* the child process */
    ...
    execCheck = execvp(parms_array[0], parms_array);
    if (execCheck == -1) 
    {
      perror("execvp"); 
      exit(-1); 
  ...
  {/* code executed by the parent process */
    ...
    while (!feof (tmpFP)) {
      ...
      //TODO rather than print to STDOUT, this will go back to client
