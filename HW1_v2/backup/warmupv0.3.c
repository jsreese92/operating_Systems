...
*/
    ...
    while(i < numOfLines){
        for(inputPointer=0; inputPointer < 80; inputPointer++){
//printf("input[%d]=%c\n",inputPointer + (i*80), input[inputPointer + (i*80)]);
           linesToPrint[i][inputPointer] = input[inputPointer + (i * 80)];
//printf("linesToPrint[%d][%d]=%c\n", i, inputPointer, linesToPrint[i][inputPointer]);
        }
    linesToPrint[i][80] = '\0';
    printf("%s\n", linesToPrint[i]);
    i++;
    }
    //for some reson, [0][81] has 'a'...
    if (linesToPrint[0][80] == '\0')
//        printf("is null character\n");
//    else
//        printf("is not null character\n");
//    printf("[0][81]=%c\n", linesToPrint[0][81]);
